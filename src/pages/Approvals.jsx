...
function Approvals() {
  ...
  const handleApprove = async (e) => {
    ...
    try {
      ...
      setSuccessMsg(
        ...
        ${response.data.approvals >= 2 ? "✅ Loan is now APPROVED!" : "⏳ Waiting for second signatory."}`
      );
      setLoanId("");
      setSignatoryId("");
    } catch (error) {
  ...
  const handleFetchApprovals = async () => {
    if (!loanId) {
      setServerError("Enter a Loan ID to view approvals");
      return;
    }
    try {
  ...
  return (
    <>
      <section className="card">
        ...
        {successMsg && (
          <p style={{ color: "green", marginBottom: "10px", whiteSpace: "pre-line" }}>
            {successMsg}
          </p>
        )}
            ...
            <tbody>
              {approvals.map((approval) => (
                <tr key={approval.approval_id}>
                  ...
                  <td>{approval.approved_at ? new Date(approval.approved_at).toLocaleDateString() : "Pending"}</td>
                </tr>
              ))}
            </tbody>
          </table>
